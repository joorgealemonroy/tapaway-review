...
async function fetchProfileData(username: string): Promise<ProfileData | null> {
  ...
  const { data: profileData, error: profileError } = await supabase
    ...
    .select('id, username, full_name, profile_photo_url, subscription_status, header_type, header_color, header_image_url, background_color, pfp_position, headline, bio, contact_enabled, contact_name, contact_photo_url, contact_phone, contact_company, contact_title, contact_address, contact_website')
  ...
  const [linksResult, blocksResult] = await Promise.all([
    supabase
      .from('personal_links')
      .select('id, link_type, label, url, pill_color, sort_order, is_active, is_featured, display_style')
