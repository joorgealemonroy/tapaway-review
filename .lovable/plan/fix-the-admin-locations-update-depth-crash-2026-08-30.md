...
3. **Clusterer created once per map instance** in an effect keyed only on `map`, with `clearMarkers()` + `setMap(null)` cleanup when the map unmounts. Because ref callbacks can attach before that effect runs, the clusterer is seeded immediately on creation with every marker already held in `markersRef`. Afterwards markers are added/removed incrementally through `addMarker`/`removeMarker` rather than a full rebuild on every change.
...
5. **Remove the `console.error` patch.** Replace it with the supported `gm_authFailure` hook (restoring any previous handler on cleanup) plus a `window.addEventListener("error", ...)` listener that stays installed until it actually matches a Google Maps error code — unrelated browser errors are ignored, not consumed — and is removed on cleanup. Both setters are guarded so an identical value returns the previous state. Diagnostic copy and all fault branches stay exactly as they are.
